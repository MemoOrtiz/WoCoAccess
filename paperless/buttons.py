#--- 1. Configuracion de Kivy (sin borde, tamaño fijo)
from kivy.config import Config
Config.set('graphics', 'borderless',  '1')
Config.set('graphics', 'resizable',   '0')
Config.set('graphics', 'width',       '300')  # ancho inicial
Config.set('graphics', 'height',      '80')   # alto inicial
...
class TouchPadApp(App):
    ...
    def build(self):
        ...
        for cfg in BUTTONS:
            ...
            elif cfg["type"] == "image_button":
                # botón que muestra SOLO imagen, no texto
                
                normal = resource_path(cfg["images"]["normal"])
                print("▶ ImageButton:", cfg["id"], "usa imagen en", normal)
                down = resource_path(cfg["images"]["down"])
                btn = Button(
                    size_hint=(None,None),
                    size = (60,60),
                    background_normal=normal,
                    background_down=down,
                    background_color=(1, 1, 1, 0),  # transparente
                    text="",  # sin texto
                )
                btn.allow_stretch = True
                btn.keep_ratio    = True
                # enlazar callback
    ...
    def on_start(self):
        ...
        # capturamos HWND y activamos color-key
        ...
        # 2) Convertir blanco en transparente
        ...
        win32gui.SetLayeredWindowAttributes(
            self.hwnd, colorkey, 0, win32con.LWA_COLORKEY
        )
